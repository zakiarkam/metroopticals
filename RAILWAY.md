...
3. Make sure `development` has **its own Postgres service**. Do not let the dev
   ...
   reference in this environment resolves to the Postgres inside *this*
   environment.
...
pointing at a database that *already* has these tables (from the old
...
connection string from the Postgres service's *Connect* tab, or run the command
