...
interface AddAdvertisementDialogProps {
  ...
  onClose: () => void;
  onSuccess: () => void;
  /** Opening from a zone card pre-selects that zone. */
  defaultPlacement?: AdvertisementPlacement;
}
...
const AddAdvertisementDialog: React.FC<AddAdvertisementDialogProps> = ({
  ...
  // Reset on every open so a cancelled draft never leaks into the next ad.
  useEffect(() => {
    if (isOpen) {
      setValues(advertisementFormDefaults(defaultPlacement));
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await createAdvertisement(result.payload);

      Toast.update(toastId, {
        render: "Advertisement created.",
        type: "success",
        isLoading: false,
        autoClose: 3000,
        closeButton: true,
      ...
      Toast.update(toastId, {
        render:
          ...
          error?.message ||
          "Could not create the advertisement. Please try again.",
        type: "error",
        isLoading: false,
        autoClose: 5000,
        closeButton: true,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-gray-3 bg-gray-2 p-0 shadow-xl sm:p-0">
        <DialogHeader className="border-b border-gray-3 px-6 py-5">
          ...
          <p className="mt-1 text-[13px] text-dark-4">
            Upload a photo, choose where it appears, and set when it runs.
          </p>
        </DialogHeader>
