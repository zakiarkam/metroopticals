...
const ProductItem = ({ item, hoverActions = false }: ProductItemProps) => {
  ...
  const resolvedImages = (() => {
    const candidates = [
      ...
      Array.isArray(item.imgs) ? item.imgs : undefined,
      item.imgs?.previews,
      item.imgs?.thumbnails,
    ];

    for (const candidate of candidates) {
      const normalized = normalizeImageArray(candidate ?? []);
  ...
  const overlayButtonClasses = hoverActions
    ? "opacity-0 scale-75 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:scale-100"
    : "";

  const imageHoverClasses = hoverActions
    ...
    <div className="w-full">
      ...
      >
          ...
          <div className="relative h-full rounded-[2px] bg-[#fbfcff] overflow-hidden">
            ...
            >
              <button
                ...
                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/40 bg-blue text-white shadow-lg transition-all duration-300 ease-out hover:bg-blue/90 hover:border-white hover:shadow-xl hover:scale-110 active:scale-95"
              >
                <svg
                  width="16"
                  height="16"
              ...
              <button
                ...
                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/40 bg-blue text-white shadow-lg transition-all duration-300 ease-out hover:bg-red hover:border-white hover:shadow-xl hover:scale-110 active:scale-95 disabled:bg-red/50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
                <svg
                  width="16"
                  height="16"
            ...
            <div className="flex h-full w-full items-center justify-center">
              <Image
                ...
                width={400}
                height={400}
                className={`h-full w-full object-contain ${imageHoverClasses}`}
              />
            </div>
          </div>
        </div>
        <div className="relative -mt-10 rounded-t-[2px] bg-white px-6 pb-6 pt-6 shadow-[0_-10px_30px_rgba(0,0,0,0.08)]">
          ...
          <h3
            className=" text-xl -mt-2 font-semibold capitalize text-dark"
          ...
          <button
            ...
            disabled={isAddingToCart || !item.stock || item.status !== "ACTIVE"}
